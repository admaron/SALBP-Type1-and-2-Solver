...
class Animation {
    constructor(t, e, i, a) {
        ...
        this._active = !0, this._fn = t.fn || interpolators[t.type || typeof s], this._easing = effects[t.easing] || effects.linear, this._start = Math.floor(Date.now() + (t.delay || 0)), this._duration = this._total = Math.floor(t.duration), this._loop = !!t.loop, this._target = e, this._prop = i, this._from = s, this._to = a, this._promises = void 0
    }
    active() {
        return this._active
    }
    update(t, e, i) {
        var a, s, n;
        this._active && (this._notify(!1), a = this._target[this._prop], s = i - this._start, n = this._duration - s, this._start = i, this._duration = Math.floor(Math.max(n, t.duration)), this._total += s, this._loop = !!t.loop, this._to = resolve([t.to, e, a, t.from]), this._from = resolve([t.from, a, e]))
    }
    cancel() {
...
class Animations {
    ...
    _createAnimations(e, i) {
        ...
        for (o = t.length - 1; 0 <= o; --o) {
            const c = t[o];
            if ("$" !== c.charAt(0))
                if ("options" === c) s.push(...this._animateOptions(e, i));
                else {
                    var l = i[c];
                    let t = n[c];
                    var h = a.get(c);
                    ...
                    h && h.duration ? (n[c] = t = new Animation(h, e, c, l), s.push(t)) : e[c] = l
                }
        }
        return s
    }
...
function convertObjectDataToArray(t) {
    ...
    let a, s, n;
    for (a = 0, s = e.length; a < s; ++a) n = e[a], i[a] = {
        x: n,
        y: t[n]
    };
    return i
...
function updateStacks(t, e) {
    ...
    for (let t = 0; t < c; ++t) {
        ...
        const f = p._stacks || (p._stacks = {}),
            m = ((d = f[l] = getOrCreateStack(s, h, u))[r] = g, d._top = getLastIndexInStack(d, n, !0, a.type), d._bottom = getLastIndexInStack(d, n, !1, a.type), d._visualValues || (d._visualValues = {}));
        m[r] = g
    }
}
...
class DatasetController {
    ...
    parse(t, e) {
        ...
        let o = 0 === t && e === a.length || i._sorted,
            l = 0 < t && i._parsed[t - 1],
            h, c, d;
        if (!1 === this._parsing) i._parsed = a, i._sorted = !0, d = a;
        else {
            d = isArray(a[t]) ? this.parseArrayData(i, a, t, e) : isObject(a[t]) ? this.parseObjectData(i, a, t, e) : this.parsePrimitiveData(i, a, t, e);
            for (h = 0; h < e; ++h) i._parsed[h + t] = c = d[h], o && ((null === c[r] || l && c[r] < l[r]) && (o = !1), l = c);
            i._sorted = o
        }
        n && updateStacks(this, d)
    }
    ...
    parseArrayData(t, e, i, a) {
        ...
        let o, l, h, c;
        for (o = 0, l = a; o < l; ++o) c = e[h = o + i], r[o] = {
            x: s.parse(c[0], h),
            y: n.parse(c[1], h)
        };
        return r
    ...
    parseObjectData(t, e, i, a) {
        ...
        const l = new Array(a);
        let h, c, d, u;
        for (h = 0, c = a; h < c; ++h) u = e[d = h + i], l[h] = {
            x: s.parse(resolveObjectKey(u, r), d),
            y: n.parse(resolveObjectKey(u, o), d)
        };
        return l
    }
    getParsed(t) {
    ...
    getMinMax(e, t) {
        ...
        function g() {
            var t = (u = a[d])[r.axis];
            return !isNumberFinite(u[e.axis]) || h > t || c < t
        }
        for (d = 0; d < n && (g() || (this.updateRangeFromParsed(l, e, u, o), !s)); ++d);
        if (s)
            for (d = n - 1; 0 <= d; --d)
                if (!g()) {
    ...
    draw() {
        ...
        for (i.dataset && i.dataset.draw(t, s, r, o), h = r; h < r + o; ++h) {
            const c = a[h];
            c.hidden || (c.active && l ? n.push(c) : c.draw(t, s))
        }
    ...
    _resolveElementOptions(t, e = "default", i) {
        ...
        const c = l.resolveNamedOptions(r, t, () => this.getContext(i, a, e), h);
        return c.$shared && (c.$shared = o, s[n] = Object.freeze(cloneIfNotShared(c, o))), c
    }
...
function computeFitCategoryTraits(t, e, i, a) {
    ...
    let n, r;
    return r = isNullOrUndef(s) ? (n = e.min * i.categoryPercentage, i.barPercentage) : (n = s * a, 1), {
        chunk: n / a,
        ratio: r,
...
function parseArrayOrPrimitive(t, e, i, a) {
    ...
    const l = [];
    let h, c, d, u;
    for (c = (h = i) + a; h < c; ++h) u = e[h], (d = {})[s.axis] = o || s.parse(r[h], h), l.push(parseValue(u, d, n, h));
    return l
}
...
function setBorderSkipped(t, e, i, a) {
    ...
    s ? !0 === s ? t.borderSkipped = {
        ...
        top: l,
        bottom: h
    } = borderProps(t), "middle" === s && i && (t.enableBorderRadius = !0, s = (i._top || 0) === a ? l : (i._bottom || 0) === a ? h : (n[parseEdge(h, e, r, o)] = !0, l)), n[parseEdge(s, e, r, o)] = !0, t.borderSkipped = n) : t.borderSkipped = n
}
...
class BarController extends DatasetController {
    ...
    parseObjectData(t, e, i, a) {
        ...
        var {
            xAxisKey: t = "x",
            yAxisKey: r = "y"
        } = this._parsing, o = "x" === s.axis ? t : r, l = "x" === n.axis ? t : r;
        const h = [];
        let c, d, u, g;
        for (d = (c = i) + a; c < d; ++c) g = e[c], (u = {})[s.axis] = s.parse(resolveObjectKey(g, o), c), h.push(parseValue(resolveObjectKey(g, l), u, n, c));
        return h
    }
    updateRangeFromParsed(t, e, i, a) {
    ...
    updateElements(e, i, a, s) {
        ...
        for (let t = i; t < i + a; t++) {
            var g = this.getParsed(t),
                p = n || isNullOrUndef(g[o.axis]) ? {
                    base: l,
                    head: l
                } : this._calculateBarValuePixels(t),
                f = this._calculateBarIndexPixels(t, c),
                m = (g._stacks || {})[o.axis];
            const v = {
                horizontal: h,
                base: p.base,
            ...
            u && (v.options = d || this.resolveDataElementOptions(t, e[t].active ? "active" : s));
            g = v.options || e[t].options;
            setBorderSkipped(v, g, m, r), setInflateAmount(v, g, c.ratio), this.updateElement(e[t], t, v, s)
        }
    }
    _getStacks(t, i) {
...
class BubbleController extends DatasetController {
    ...
    updateElements(e, i, a, s) {
        ...
        var {
            sharedOptions: l,
            includeOptions: h
        } = this._getSharedOptions(i, s), c = r.axis, d = o.axis;
        for (let t = i; t < i + a; t++) {
            var u = e[t],
                g = !n && this.getParsed(t);
            const f = {};
            var p = f[c] = n ? r.getPixelForDecimal(.5) : r.getPixelForValue(g[c]),
                g = f[d] = n ? o.getBasePixel() : o.getPixelForValue(g[d]);
...
function getRatioAndOffset(t, e, a) {
    ...
    if (e < TAU) {
        ...
        var t = Math.cos(u),
            ...
            c = (t, e, i) => _angleBetween(t, u, g, !0) ? -1 : Math.min(e, e * a, i, i * a),
            d = h(0, t, o),
            h = h(HALF_PI, e, l),
            t = c(PI, t, o),
            o = c(PI + HALF_PI, e, l);
        i = (d - t) / 2, s = (h - o) / 2, n = -(d + t) / 2, r = -(h + o) / 2
    }
    return {
        ratioX: i,
        ratioY: s,
...
class DoughnutController extends DatasetController {
    ...
    update(t) {
        ...
        this.offsetX = h * o, this.offsetY = c * o, i.total = this.calculateTotal(), this.outerRadius = o - l * this._getRingWeightOffset(this.index), this.innerRadius = Math.max(this.outerRadius - l * r, 0), this.updateElements(a, 0, a.length, t)
    }
    _circumference(t, e) {
        var i = this.options,
            a = this._cachedMeta,
    ...
    updateElements(t, e, i, a) {
        ...
        for (p = e; p < e + i; ++p) {
            ...
            u && (v.options = d || this.resolveDataElementOptions(p, m.active ? "active" : a)), g += f, this.updateElement(m, p, v, a)
        }
    }
    calculateTotal() {
        var t = this._cachedMeta,
...
class LineController extends DatasetController {
    ...
    update(t) {
        ...
        let {
            start: r,
            count: o
        } = _getStartAndCountOfVisiblePoints(e, a, n);
    ...
    updateElements(e, i, t, a) {
        ...
        var {
            sharedOptions: h,
            includeOptions: c
        } = this._getSharedOptions(i, a), d = n.axis, u = r.axis, {
            spanGaps: g,
            segment: p
        } = this.options, f = isNumber(g) ? g : Number.POSITIVE_INFINITY, m = this.chart._animationsDisabled || s || "none" === a, v = i + t, x = e.length;
        let b = 0 < i && this.getParsed(i - 1);
        for (let t = 0; t < x; ++t) {
            var _, y, k, S, D = e[t];
            const M = m ? D : {};
            t < i || t >= v ? M.skip = !0 : (_ = this.getParsed(t), y = isNullOrUndef(_[u]), k = M[d] = n.getPixelForValue(_[d], t), S = M[u] = s || y ? r.getBasePixel() : r.getPixelForValue(o ? this.applyStack(r, _, o) : _[u], t), M.skip = isNaN(k) || isNaN(S) || y, M.stop = 0 < t && Math.abs(_[d] - b[d]) > f, p && (M.parsed = _, M.raw = l.data[t]), c && (M.options = h || this.resolveDataElementOptions(t, D.active ? "active" : a)), m || this.updateElement(D, t, M, a), b = _)
        }
    }
    getMaxOverflow() {
        var t = this._cachedMeta,
...
class PolarAreaController extends DatasetController {
    ...
    updateElements(a, t, e, s) {
        ...
        for (g = t; g < t + e; g++) {
            ...
            u = e, n && (o.animateScale && (i = 0), o.animateRotate && (t = e = d));
            var m = {
                x: h,
                y: c,
                innerRadius: 0,
...
class ScatterController extends DatasetController {
    ...
    update(t) {
        ...
        let {
            start: s,
            count: n
        } = _getStartAndCountOfVisiblePoints(e, i, a);
        if (this._drawStart = s, this._drawCount = n, _scaleRangesChanged(e) && (s = 0, n = i.length), this.options.showLine) {
            const {
                dataset: r,
                _dataset: o
            } = e, l = (r._chart = this.chart, r._datasetIndex = this.index, r._decimated = !!o._decimated, r.points = i, this.resolveDatasetElementOptions(t));
    ...
    updateElements(e, i, a, s) {
        ...
        for (let t = i; t < i + a; ++t) {
            ...
            D.skip = isNaN(k) || isNaN(S) || y, D.stop = 0 < t && Math.abs(_[u] - x[u]) > m, f && (D.parsed = _, D.raw = h.data[t]), d && (D.options = c || this.resolveDataElementOptions(t, b.active ? "active" : s)), v || this.updateElement(b, t, D, s), x = _
        }
        this.updateSharedOptions(c, s, t)
    }
    getMaxOverflow() {
        var t = this._cachedMeta;
        const i = t.data || [];
...
function getIntersectItems(a, s, t, n, r) {
    ...
    return evaluateInteractionItems(a, t, s, function (t, e, i) {
        (r || _isPointInArea(t, a.chartArea, 0)) && t.inRange(s.x, s.y, n) && o.push({
            element: t,
            datasetIndex: e,
            index: i
        })
...
function getNearestRadialItems(t, r, e, o) {
    ...
    return evaluateInteractionItems(t, e, r, function (t, e, i) {
        var {
            startAngle: a,
            endAngle: s
        } = t.getProps(["startAngle", "endAngle"], o), n = getAngleFromPoint(t, {
            x: r.x,
            y: r.y
        })["angle"];
        _angleBetween(n, a, s) && l.push({
            element: t,
            datasetIndex: e,
            index: i
        })
...
function getNearestCartesianItems(n, r, t, o, l, h) {
    let c = [];
    const d = getDistanceMetricForAxis(t);
    let u = Number.POSITIVE_INFINITY;
    return evaluateInteractionItems(n, t, r, function (t, e, i) {
        var a, s = t.inRange(r.x, r.y, l);
        o && !s || (a = t.getCenterPoint(l), (h || n.isPointInArea(a) || s) && ((s = d(r, a)) < u ? (c = [{
            element: t,
            datasetIndex: e,
            index: i
        }], u = s) : s === u && c.push({
...
function getAxisItems(t, a, s, e, n) {
    ...
    let l = !1;
    return evaluateInteractionItems(t, s, a, (t, e, i) => {
        t[o](a[s], n) && (r.push({
            element: t,
            datasetIndex: e,
            index: i
    ...
    modes: {
        ...
        dataset(t, e, i, a) {
            ...
            if (0 < r.length) {
                ...
                r = [];
                for (let t = 0; t < l.length; ++t) r.push({
                    element: l[t],
                    datasetIndex: o,
                    index: t
                })
...
function buildLayoutBoxes(t) {
    const e = wrapBoxes(t);
    t = sortByWeight(e.filter(t => t.box.fullSize), !0);
...
function fitBoxes(t, e, i, a) {
    ...
    for (n = 0, r = t.length, h = 0; n < r; ++n) {
        ...
        h |= d && s.length, c = c || u, l.fullSize || s.push(o)
    }
    return h && fitBoxes(s, e, i, a) || c
}
    ...
    update(i, t, e, a) {
        if (i) {
            ...
            var l = o.horizontal,
                ...
                }), c.reduce((t, e) => e.box.options && !1 === e.box.options.display ? t : t + 1, 0) || 1),
                t = Object.freeze({
                    outerWidth: t,
                    outerHeight: e,
                    padding: s,
            ...
            h = setLayoutDims(c.concat(l), t);
            fitBoxes(o.fullSize, d, t, h), fitBoxes(c, d, t, h), fitBoxes(l, d, t, h) && fitBoxes(c, d, t, h), handleMaxPadding(d), placeBoxes(o.leftAndTop, d, t, h), d.x += d.w, d.y += d.h, placeBoxes(o.rightAndBottom, d, t, h), i.chartArea = {
                left: d.left,
                top: d.top,
                right: d.left + d.w,
                bottom: d.top + d.h,
...
function initCanvas(t, e) {
    ...
    }, i.display = i.display || "block", i.boxSizing = i.boxSizing || "border-box", isNullOrEmpty(s) && void 0 !== (s = readUsedSize(t, "width")) && (t.width = s), isNullOrEmpty(a) && ("" === t.style.height ? t.height = t.width / (e || 2) : void 0 !== (s = readUsedSize(t, "height")) && (t.height = s)), t
}
...
function onWindowResize() {
    ...
    i !== oldDevicePixelRatio && (oldDevicePixelRatio = i, drpListeningCharts.forEach((t, e) => {
...
function skip(t, e, i, a, s) {
    ...
    let o = 0,
        l, h, c;
    for (i = Math.ceil(i), s && (i = (l = s - a) / Math.floor(l / i)), c = n; c < 0;) o++, c = Math.round(n + o * i);
    for (h = Math.max(n, 0); h < r; h++) h === c && (e.push(t[h]), o++, c = Math.round(n + o * i))
}
...
function titleArgs(t, e, i, a) {
    ...
    var f, m, l = r - s,
        v = o - n;
    return t.isHorizontal() ? (g = _alignStartEnd(a, n, o), p = isObject(i) ? (m = i[f = Object.keys(i)[0]], c[f].getPixelForValue(m) + l - e) : "center" === i ? (h.bottom + h.top) / 2 + l - e : offsetFromEdge(t, i, e), u = o - n) : (g = isObject(i) ? (m = i[f = Object.keys(i)[0]], c[f].getPixelForValue(m) - v + e) : "center" === i ? (h.left + h.right) / 2 - v + e : offsetFromEdge(t, i, e), p = _alignStartEnd(a, r, s), d = "left" === i ? -HALF_PI : HALF_PI), {
        titleX: g,
        titleY: p,
        maxWidth: u,
        rotation: d
...
class Scale extends Element {
    ...
    getUserBounds() {
        var {
            ...
            _suggestedMin: i,
            _suggestedMax: a
        } = this, t = finiteOrDefault(t, Number.POSITIVE_INFINITY), e = finiteOrDefault(e, Number.NEGATIVE_INFINITY), i = finiteOrDefault(i, Number.POSITIVE_INFINITY), a = finiteOrDefault(a, Number.NEGATIVE_INFINITY);
        return {
    ...
    update(t, e, i) {
        var {
            ...
            grace: s,
            ticks: n
        } = this.options, r = n.sampleSize, t = (this.beforeUpdate(), this.maxWidth = t, this.maxHeight = e, this._margins = i = Object.assign({
            left: 0,
            right: 0,
            top: 0,
            bottom: 0
        }, i), this.ticks = null, this._labelSizes = null, this._gridLineItems = null, this._labelItems = null, this.beforeSetDimensions(), this.setDimensions(), this.afterSetDimensions(), this._maxLength = this.isHorizontal() ? this.width + i.left + i.right : this.height + i.top + i.bottom, this._dataLimitsCached || (this.beforeDataLimits(), this.determineDataLimits(), this.afterDataLimits(), this._range = _addGrace(this, s, a), this._dataLimitsCached = !0), this.beforeBuildTicks(), this.ticks = this.buildTicks() || [], this.afterBuildTicks(), r < this.ticks.length);
    ...
    fit() {
        ...
        var e, i, a, s, n, {
            ...
            c = this._isVisible(),
            d = this.isHorizontal();
        c && (c = getTitleHeight(l, r.options.font), d ? (t.width = this.maxWidth, t.height = getTickMarkLength(h) + c) : (t.height = this.maxHeight, t.width = getTickMarkLength(h) + c), o.display && this.ticks.length && ({
            first: l,
            last: h,
            widest: c,
            highest: e
        } = this._getLabelSizes(), i = 2 * o.padding, s = toRadians(this.labelRotation), a = Math.cos(s), s = Math.sin(s), d ? (n = o.mirror ? 0 : s * c.width + a * e.height, t.height = Math.min(this.maxHeight, t.height + n + i)) : (n = o.mirror ? 0 : a * c.width + s * e.height, t.width = Math.min(this.maxWidth, t.width + n + i)), this._calculatePadding(l, h, s, a))), this._handleMargins(), d ? (this.width = this._length = r.width - this._margins.left - this._margins.right, this.height = t.height) : (this.width = t.width, this.height = this._length = r.height - this._margins.top - this._margins.bottom)
    }
    _calculatePadding(i, a, s, n) {
        var {
            ...
            position: l
        } = this.options, h = 0 !== this.labelRotation, l = "top" !== l && "x" === this.axis;
        if (this.isHorizontal()) {
            var c = this.getPixelForTick(0) - this.left,
                d = this.right - this.getPixelForTick(this.ticks.length - 1);
            let t = 0,
                e = 0;
            h ? e = l ? (t = n * i.width, s * a.height) : (t = s * i.height, n * a.width) : "start" === r ? e = a.width : "end" === r ? t = i.width : "inner" !== r && (t = i.width / 2, e = a.width / 2), this.paddingLeft = Math.max((t - c + o) * this.width / (this.width - c), 0), this.paddingRight = Math.max((e - d + o) * this.width / (this.width - d), 0)
        } else {
            let t = a.height / 2,
                e = i.height / 2;
    ...
    _getLabelSizes() {
        ...
        if (!e) {
            ...
            let t = this.ticks;
            i < t.length && (t = sample(t, i)), this._labelSizes = e = this._computeLabelSizes(t, t.length)
        }
        return e
    }
    _computeLabelSizes(t, e) {
        ...
        let r = 0,
            o = 0,
            l, h, c, d, u, g, p, f, m, v, x;
        for (l = 0; l < e; ++l) {
            if (d = t[l].label, u = this._resolveTickFontOptions(l), i.font = g = u.string, p = a[g] = a[g] || {
                    data: {},
                    gc: []
                }, f = u.lineHeight, m = v = 0, isNullOrUndef(d) || isArray(d)) {
                if (isArray(d))
                    for (h = 0, c = d.length; h < c; ++h) x = d[h], isNullOrUndef(x) || isArray(x) || (m = _measureText(i, p.data, p.gc, m, x), v += f)
            } else m = _measureText(i, p.data, p.gc, m, d), v = f;
            s.push(m), n.push(v), r = Math.max(m, r), o = Math.max(v, o)
        }
        garbageCollect(a, e);
        var b = s.indexOf(r),
            _ = n.indexOf(o),
    ...
    _computeGridLineItems(t) {
        ...
        var f, m, u = g / 2;
        let v, x, b, _, y, k, S, D, M, P, w, C;
        "top" === n ? (v = p(this.bottom), k = this.bottom - c, D = v - u, P = p(t.top) + u, C = t.bottom) : "bottom" === n ? (v = p(this.top), P = t.top, C = p(t.bottom) - u, k = v + u, D = this.top + c) : "left" === n ? (v = p(this.right), y = this.right - c, S = v - u, M = p(t.left) + u, w = t.right) : "right" === n ? (v = p(this.left), M = t.left, w = p(t.right) - u, y = v + u, S = this.left + c) : "x" === e ? ("center" === n ? v = p((t.top + t.bottom) / 2 + .5) : isObject(n) && (m = n[f = Object.keys(n)[0]], v = p(this.chart.scales[f].getPixelForValue(m))), P = t.top, C = t.bottom, k = v + u, D = k + c) : "y" === e && ("center" === n ? v = p((t.left + t.right) / 2) : isObject(n) && (m = n[f = Object.keys(n)[0]], v = p(this.chart.scales[f].getPixelForValue(m))), y = v - u, S = y - c, M = t.left, w = t.right);
        var e = valueOrDefault(a.ticks.maxTicksLimit, h),
            A = Math.max(1, Math.ceil(h / e));
        for (x = 0; x < h; x += A) {
            ...
            void 0 !== (b = getPixelForGridLine(this, x, o)) && (_ = _alignPixel(i, b, T), l ? y = S = M = w = _ : k = D = P = C = _, d.push({
                tx1: y,
                ty1: k,
                tx2: S,
                ty2: D,
        ...
        return this._ticksLength = h, this._borderValue = v, d
    }
    _computeLabelItems(t) {
        ...
        const p = [];
        let f, m, v, x, b, _, y, k, S, D, M, P = "middle";
        "top" === a ? (b = this.bottom - c, _ = this._getXAxisLabelAlignment()) : "bottom" === a ? (b = this.top + c, _ = this._getXAxisLabelAlignment()) : "left" === a ? (c = this._getYAxisLabelAlignment(i), _ = c.textAlign, x = c.x) : "right" === a ? (c = this._getYAxisLabelAlignment(i), _ = c.textAlign, x = c.x) : "x" === e ? ("center" === a ? b = (t.top + t.bottom) / 2 + u : isObject(a) && (n = a[c = Object.keys(a)[0]], b = this.chart.scales[c].getPixelForValue(n) + u), _ = this._getXAxisLabelAlignment()) : "y" === e && ("center" === a ? x = (t.left + t.right) / 2 - u : isObject(a) && (n = a[c = Object.keys(a)[0]], x = this.chart.scales[c].getPixelForValue(n)), _ = this._getYAxisLabelAlignment(i).textAlign), "y" === e && ("start" === l ? P = "top" : "end" === l && (P = "bottom"));
        var w = this._getLabelSizes();
        for (f = 0, m = o.length; f < m; ++f) {
            v = o[f].label;
            var C = s.setContext(this.getContext(f)),
                A = (y = this.getPixelForTick(f) + s.labelOffset, S = (k = this._resolveTickFontOptions(f)).lineHeight, (D = isArray(v) ? v.length : 1) / 2),
                O = C.color,
                L = C.textStrokeColor,
                T = C.textStrokeWidth;
            let t = _;
            r ? (x = y, "inner" === _ && (t = f === m - 1 ? this.options.reverse ? "left" : "right" : 0 === f ? this.options.reverse ? "right" : "left" : "center"), M = "top" === a ? "near" === h || 0 != g ? -D * S + S / 2 : "center" === h ? -w.highest.height / 2 - A * S + S : -w.highest.height + S / 2 : "near" === h || 0 != g ? S / 2 : "center" === h ? w.highest.height / 2 - A * S : w.highest.height - D * S, d && (M *= -1), 0 == g || C.showLabelBackdrop || (x += S / 2 * Math.sin(g))) : (b = y, M = (1 - D) * S / 2);
            let i;
            if (C.showLabelBackdrop) {
                var A = toPadding(C.backdropPadding),
                    E = w.heights[f],
                    I = w.widths[f];
                let t = M - A.top,
                    e = 0 - A.left;
    ...
    _getYAxisLabelAlignment(t) {
        ...
        } = this.options, t = t + s, n = this._getLabelSizes().widest.width;
        let r, o;
    ...
    drawBorder() {
        ...
        if (l) {
            var h = r.setContext(this.getContext(0)).lineWidth,
                c = this._borderValue;
            let t, e, i, a;
            this.isHorizontal() ? (t = _alignPixel(s, this.left, l) - l / 2, e = _alignPixel(s, this.right, h) + h / 2, i = a = c) : (i = _alignPixel(s, this.top, l) - l / 2, a = _alignPixel(s, this.bottom, h) + h / 2, t = e = c), n.save(), n.lineWidth = o.width, n.strokeStyle = o.color, n.beginPath(), n.moveTo(t, i), n.lineTo(e, a), n.stroke(), n.restore()
        }
    }
    drawLabels(t) {
...
class Registry {
    ...
    _each(i, t, a) {
        [...t].forEach(t => {
            ...
            a || e.isForType(t) || e === this.plugins && t.id ? this._exec(i, e, t) : each(t, t => {
...
function allPlugins(t) {
    ...
    var s = t.plugins || [];
    for (let t = 0; t < s.length; t++) {
        var n = s[t]; - 1 === i.indexOf(n) && (i.push(n), e[n.id] = !0)
    }
    return {
        plugins: i,
        localIds: e
...
function determineAxis(t, e) {
    ...
    if (t = e.axis || axisFromPosition(e.position) || 1 < t.length && determineAxis(t[0].toLowerCase(), e)) return t;
    throw new Error(`Cannot determine type of '${name}' axis. Please provide 'axis' or 'position' option.`)
}

function mergeScaleConfig(e, i) {
    const n = overrides[e.type] || {
            scales: {}
        },
        r = i.scales || {},
        o = getIndexAxis(e.type, i),
    ...
    return Object.keys(r).forEach(t => {
        ...
        var i = determineAxis(t, e),
            a = getDefaultScaleIDFromAxis(i, o),
            s = n.scales || {};
        l[t] = mergeIf(Object.create(null), [{
            axis: i
        }, e, s[i], s[a]])
    }), e.data.datasets.forEach(a => {
        var t = a.type || e.type;
        const s = a.indexAxis || getIndexAxis(t, i),
            n = (overrides[t] || {}).scales || {};
        Object.keys(n).forEach(t => {
            var e = getAxisFromDefaultScaleID(t, s),
                i = a[e + "AxisID"] || e;
            l[i] = l[i] || Object.create(null), mergeIf(l[i], [{
                axis: e
            }, r[i], n[t]])
        })
...
class Config {
    ...
    getOptionScopes(e, t, i) {
        const {
            options: a,
            type: s
        } = this, n = this._cachedScopes(e, i);
        i = n.get(t);
        if (i) return i;
        const r = new Set,
            o = (t.forEach(t => {
                e && (r.add(e), t.forEach(t => addIfFound(r, e, t))), t.forEach(t => addIfFound(r, a, t)), t.forEach(t => addIfFound(r, overrides[s] || {}, t)), t.forEach(t => addIfFound(r, defaults, t)), t.forEach(t => addIfFound(r, descriptors, t))
            }), Array.from(r));
    ...
    chartOptionScopes() {
        ...
        return [t, overrides[e] || {}, defaults.datasets[e] || {}, {
            type: e
...
function getResolver(t, e, i) {
    ...
    t = i.join();
    let s = a.get(t);
    return s || (e = _createResolver(e, i), s = {
        resolver: e,
...
class Chart {
    ...
    _resize(t, e) {
        ...
        this.width = a.width, this.height = a.height, this._aspectRatio = this.aspectRatio, retinaScale(this, t, !0) && (this.notifyPlugins("resize", {
            size: a
        }), callback(i.onResize, [this, a], this), this.attached && this._doResize(e) && this.render())
    }
    ensureScalesHaveIDs() {
    ...
    buildOrUpdateScales() {
        ...
        let t = [];
        a && (t = t.concat(Object.keys(a).map(t => {
            var e = a[t],
                t = determineAxis(t, e),
                i = "r" === t,
                t = "x" === t;
    ...
    buildOrUpdateControllers() {
        ...
        for (this._removeUnreferencedMetasets(), a = 0, t = i.length; a < t; a++) {
            ...
            if (t.type && t.type !== n && (this._destroyDatasetMeta(a), t = this.getDatasetMeta(a)), t.type = n, t.indexAxis = s.indexAxis || getIndexAxis(n, this.options), t.order = s.order || 0, t.index = a, t.label = "" + s.label, t.visible = this.isDatasetVisible(a), t.controller) t.controller.updateIndex(a), t.controller.linkScales();
            else {
                ...
                }), t.controller = new r(this, a), e.push(t.controller)
            }
        }
    ...
    update(t) {
        ...
        if (this._updateScales(), this._checkEventBindings(), this._updateHiddenIndices(), this._plugins.invalidate(), !1 !== this.notifyPlugins("beforeUpdate", {
            ...
            i = this._minPadding = a.layout.autoPadding ? i : 0, this._updateLayout(i), s || each(r, t => {
                t.reset()
    ...
    _updateHiddenIndices() {
        var t, e, i, a = this["_hiddenIndices"];
        for ({
                method: t,
                start: e,
                count: i
    ...
    _updateDatasets(i) {
        if (!1 !== this.notifyPlugins("beforeDatasetsUpdate", {
            ...
            for (let t = 0, e = this.data.datasets.length; t < e; ++t) this._updateDataset(t, isFunction(i) ? i({
                datasetIndex: t
            }) : i);
    ...
    _drawDataset(t) {
        ...
        !1 !== this.notifyPlugins("beforeDatasetDraw", n) && (a && clipArea(e, {
        ...
        }), t.controller.draw(), a && unclipArea(e), n.cancelable = !1, this.notifyPlugins("afterDatasetDraw", n))
    }
    isPointInArea(t) {
    ...
    _updateVisibility(e, t, i) {
        ...
        defined(t) ? (s.data[t].hidden = !i, this.update()) : (this.setDatasetVisibility(e, i), n.update(s, {
            visible: i
    ...
    _handleEvent(t, e, i) {
        var {
            _active: a = [],
            options: s
        } = this, n = this._getActiveElements(t, a, i, e), r = _isClickEvent(t), o = determineLastEvent(t, this._lastEvent, i, r), i = (i && (this._lastEvent = null, callback(s.onHover, [t, n, this], this), r && callback(s.onClick, [t, n, this], this)), !_elementsEqual(n, a));
...
function pathArc(t, e, i, a, s, n) {
    var {
        ...
        pixelMargin: h,
        innerRadius: c
    } = e, d = Math.max(e.outerRadius + a + i - h, 0), h = 0 < c ? c + a + i + h : 0;
    let u = 0;
    var g = s - l,
        c = (a && (c = ((0 < c ? c - a : 0) + (0 < d ? d - a : 0)) / 2, u = (g - (0 != c ? g * c / (c + a) : g)) / 2), Math.max(.001, g * d - i / PI) / d),
        a = (g - c) / 2,
        i = l + a + u,
        g = s - a - u,
        {
...
function drawBorder(e, i, a, s, n) {
    var {
        ...
        borderWidth: c,
        borderJoinStyle: d
    } = h, h = "inner" === h.borderAlign;
    if (c) {
        h ? (e.lineWidth = 2 * c, e.lineJoin = d || "round") : (e.lineWidth = c, e.lineJoin = d || "bevel");
        let t = i.endAngle;
        if (r) {
            pathArc(e, i, a, s, t, n);
...
class ArcElement extends Element {
    ...
    getCenterPoint(t) {
        var {
            ...
            offset: r,
            spacing: o
        } = this.options, i = (i + a) / 2, a = (s + n + o + r) / 2;
        return {
            x: t + Math.cos(i) * a,
            y: e + Math.sin(i) * a
        }
...
function pathSegment(t, e, i, a) {
    ...
    for (u = 0; u <= l; ++u)(g = s[(o + (d ? l - u : u)) % r]).skip || (c ? (t.moveTo(g.x, g.y), c = !1) : h(t, p, g, d, n.stepped), p = g);
    return e && (g = s[(o + (d ? l : 0)) % r], h(t, p, g, d, n.stepped)), !!e
}
...
function fastPathSegment(t, e, i, a) {
    ...
    let c = 0,
        d = 0,
        u, g, p, f, m, v;
    var x, b, _, y = t => (r + (h ? o - t : t)) % n,
        k = () => {
            f !== m && (t.lineTo(c, m), t.lineTo(c, f), t.lineTo(c, v))
        };
    for (l && (g = s[y(0)], t.moveTo(g.x, g.y)), u = 0; u <= o; ++u)(g = s[y(u)]).skip || (x = g.x, b = g.y, (_ = 0 | x) === p ? (b < f ? f = b : b > m && (m = b), c = (d * c + x) / ++d) : (k(), t.lineTo(x, b), p = _, d = 0, f = m = b), v = b);
    k()
}
...
function strokePathDirect(t, e, i, a) {
    ...
    for (const o of s) setStyle(t, n, o.style), t.beginPath(), r(t, e, o, {
        start: i,
        end: i + a - 1
        ...
        if (o.length) {
            ...
            for (t = 0, e = o.length; t < e; ++t) {
                ...
                if (l === h) d.push(l);
                    ...
                    var c = Math.abs((n - l[a]) / (h[a] - l[a]));
                    const g = u(l, h, c, s.stepped);
                    g[a] = i[a], d.push(g)
                }
            }
...
function getBarBounds(t, e) {
    ...
    let r, o, l, h, c;
    return h = t.horizontal ? (c = n / 2, r = Math.min(e, a), o = Math.max(e, a), l = i - c, i + c) : (c = s / 2, r = e - c, o = e + c, l = Math.min(i, a), Math.max(i, a)), {
        left: r,
        top: l,
        right: o,
        bottom: h
...
function lttbDecimation(a, s, n, t, e) {
    ...
    let h = s,
        c, d, u, g, p;
    for (r[l++] = a[h], c = 0; c < i - 2; c++) {
        let t = 0,
            e = 0,
            i;
        var f = Math.floor((c + 1) * o) + 1 + s,
            m = Math.min(Math.floor((c + 2) * o) + 1, n) + s,
            v = m - f;
        for (i = f; i < m; i++) t += a[i].x, e += a[i].y;
        t /= v, e /= v;
        var f = Math.floor(c * o) + 1 + s,
            x = Math.min(Math.floor((c + 1) * o) + 1, n) + s,
            {
                x: b,
                y: _
            } = a[h];
        for (u = -1, i = f; i < x; i++)(g = .5 * Math.abs((b - t) * (a[i].y - _) - (b - a[i].x) * (e - _))) > u && (u = g, d = a[i], p = i);
        r[l++] = d, h = p
    }
    return r[l++] = a[e], r
}
...
function minMaxDecimation(t, e, i, a) {
    ...
    for (r = e; r < e + i; ++r) {
        ...
        var b, _, y, k = 0 | l;
        k === c ? (h < p ? (p = h, d = r) : h > f && (f = h, u = r), s = (n * s + o.x) / ++n) : (b = r - 1, isNullOrUndef(d) || isNullOrUndef(u) || (_ = Math.min(d, u), y = Math.max(d, u), _ !== g && _ !== b && m.push({
            ...t[_],
            x: s
        }), y !== g && y !== b && m.push({
            ...t[y],
            x: s
        })), 0 < r && b !== g && m.push(t[b]), m.push(o), c = k, n = 0, p = f = h, d = u = g = r)
    }
    return m
}
...
function getStartAndCountOfVisiblePointsSimplified(t, e) {
    ...
    return o && (a = _limitValue(_lookupByKey(e, n.axis, t).lo, 0, i - 1)), s = l ? _limitValue(_lookupByKey(e, n.axis, r).hi + 1, a, i) - a : i - a, {
        start: a,
        count: s
    }
}
        ...
        if (l.enabled) {
            ...
            o.data.datasets.forEach((e, t) => {
                var {
                    _data: i,
                    indexAxis: a
                } = e, t = o.getDatasetMeta(t), s = i || e.data;
                if ("y" !== resolve([a, o.options.indexAxis]) && t.controller.supportsDecimation) {
                    ...
                    if (("linear" === a.type || "time" === a.type) && !o.options.parsing) {
                        ...
                        if (r <= (l.threshold || 4 * h)) cleanDecimatedDataset(e);
                            ...
                            isNullOrUndef(i) && (e._data = s, delete e.data, Object.defineProperty(e, "data", {
                                configurable: !0,
                                enumerable: !0,
                                get: function () {
...
function _segments(t, e, i) {
    ...
    for (const d of a) {
        ...
        } = d, l = _findSegmentEnd(o, l, s), h = _getBounds(i, s[o], s[l], d.loop);
        if (e.segments)
            for (const u of _boundSegments(e, h)) {
                var c = _getBounds(i, n[u.start], n[u.end], u.loop);
                for (const g of _boundSegment(d, s, c)) r.push({
                    source: g,
                    target: u,
                    start: {
                        [i]: _getEdge(h, c, "start", Math.max)
                    },
                    end: {
                        [i]: _getEdge(h, c, "end", Math.min)
                    }
                })
            } else r.push({
                source: d,
...
function _getBounds(i, a, s, t) {
    if (!t) {
        let t = a[i],
            e = s[i];
        return "angle" === i && (t = _normalizeAngle(t), e = _normalizeAngle(e)), {
            property: i,
            start: t,
            end: e
        }
...
function _createBoundaryLine(t, e) {
    let i = [],
        a = !1;
    return (i = isArray(t) ? (a = !0, t) : _pointsFromSegments(t, e)).length ? new LineElement({
        points: i,
        options: {
            tension: 0
        },
...
function _drawfill(t, e, i) {
    ...
    a && e.points.length && (clipArea(t, i), doFill(t, {
        line: e,
        target: a,
        above: o,
        below: r,
...
function clipVertical(t, e, i) {
    ...
    for (const c of a) {
        ...
        } = c, h = s[o], o = s[_findSegmentEnd(o, l, s)];
        n ? (t.moveTo(h.x, h.y), n = !1) : (t.lineTo(h.x, i), t.lineTo(h.x, h.y)), (r = !!e.pathSegment(t, c, {
            move: r
...
function fill(e, t) {
    ...
    for ({
        ...
        } = o, u = !0 !== a, d = (e.save(), e.fillStyle = d, clipBounds(e, r, u && _getBounds(s, h, c)), e.beginPath(), !!i.pathSegment(e, o));
        let t;
        u && (d ? e.closePath() : interpolatedLineTo(e, a, c, s), u = !!a.pathSegment(e, l, {
            move: d,
            reverse: !0
        }), (t = d && u) || interpolatedLineTo(e, a, h, s)), e.closePath(), e.fill(t ? "evenodd" : "nonzero"), e.restore()
    }
}
...
var index = {
    ...
    afterDatasetsUpdate(t, e, i) {
        ...
        const s = [];
        let n, r, o, l;
        for (r = 0; r < a; ++r) o = (n = t.getDatasetMeta(r)).dataset, l = null, o && o.options && o instanceof LineElement && (l = {
...
class Legend extends Element {
    ...
    _fitRows(t, i, a, s) {
        ...
        } = this, l = this.legendHitBoxes = [], h = this.lineWidths = [0], c = s + o;
        let d = t,
            u = (n.textAlign = "left", n.textBaseline = "middle", -1),
            g = -c;
        return this.legendItems.forEach((t, e) => {
            t = a + i / 2 + n.measureText(t.text).width;
            (0 === e || h[h.length - 1] + t + 2 * o > r) && (d += c, h[h.length - (0 < e ? 0 : 1)] = 0, g += c, u++), l[e] = {
                left: 0,
                top: g,
                row: u,
                width: t,
    ...
    _fitCols(t, a, s, n) {
        ...
        } = this, l = this.legendHitBoxes = [], h = this.columnSizes = [], c = e - t;
        let d = o,
            u = 0,
            g = 0,
            p = 0,
        ...
        return this.legendItems.forEach((t, e) => {
            var {
                itemWidth: t,
                itemHeight: i
            } = calculateItemSize(s, a, r, t, n);
            0 < e && g + i + 2 * o > c && (d += u + o, h.push({
                width: u,
                height: g
            }), p += u + o, f++, u = g = 0), l[e] = {
                left: p,
    ...
    adjustHitBoxes() {
        if (this.options.display) {
            ...
            if (this.isHorizontal()) {
                ...
                for (const l of a) l.col !== t && (t = l.col, e = _alignStartEnd(s, this.top + i + n, this.bottom - this.columnSizes[t].height)), l.top = e, l.left += this.left + n, l.left = r.leftForLtr(r.x(l.left), l.width), e += l.height + n
            }
        }
    }
    isHorizontal() {
    ...
    _draw() {
        const {
            ...
            align: m,
            labels: v
        } = u, x = defaults.color, b = getRtlAdapter(u.rtl, this.left, this.width), _ = toFont(v.font), y = v["padding"], k = _.size, S = k / 2;
        let D;
        ...
        const {
            ...
            boxHeight: P,
            itemHeight: w
        } = getBoxSize(v, k), C = this.isHorizontal(), A = this._computeTitleHeight(), O = (D = C ? {
            x: _alignStartEnd(m, this.left + y, this.right - p[0]),
            y: this.top + y + A,
            line: 0
        } : {
            x: this.left + y,
            y: _alignStartEnd(m, this.top + A + y, this.bottom - g[0].height),
            line: 0
        }, overrideTextDirection(this.ctx, u.textDirection), w + y);
        this.legendItems.forEach((t, e) => {
            ...
            let s = D.x,
                n = D.y;
            b.setWidth(this.width), C ? 0 < e && s + i + y > this.right && (n = D.y += O, D.line++, s = D.x = _alignStartEnd(m, this.left + y, this.right - p[D.line])) : 0 < e && n + O > this.bottom && (s = D.x = s + g[D.line].width + y, D.line++, n = D.y = _alignStartEnd(m, this.top + A + y, this.bottom - g[D.line].height));
            var r, o, l, h, c, d, e = b.x(s);
            e = e, h = n, c = t, isNaN(M) || M <= 0 || isNaN(P) || P < 0 || (f.save(), r = valueOrDefault(c.lineWidth, 1), f.fillStyle = valueOrDefault(c.fillStyle, x), f.lineCap = valueOrDefault(c.lineCap, "butt"), f.lineDashOffset = valueOrDefault(c.lineDashOffset, 0), f.lineJoin = valueOrDefault(c.lineJoin, "miter"), f.lineWidth = r, f.strokeStyle = valueOrDefault(c.strokeStyle, x), f.setLineDash(valueOrDefault(c.lineDash, [])), v.usePointStyle ? (o = {
                radius: P * Math.SQRT2 / 2,
                pointStyle: c.pointStyle,
                rotation: c.rotation,
                borderWidth: r
            }, l = b.xPlus(e, M / 2), d = h + S, drawPointLegend(f, o, l, d, v.pointStyleWidth && M)) : (o = h + Math.max((k - P) / 2, 0), l = b.leftForLtr(e, M), d = toTRBLCorners(c.borderRadius), f.beginPath(), Object.values(d).some(t => 0 !== t) ? addRoundedRectPath(f, {
                x: l,
                y: o,
                w: M,
                h: P,
                radius: d
            }) : f.rect(l, o, M, P), f.fill(), 0 !== r && f.stroke()), f.restore()), s = _textX(a, s + M + S, C ? s + i : this.right, u.rtl), h = b.x(s), e = n, c = t, renderText(f, c.text, h, e + w / 2, _, {
                strikethrough: c.hidden,
                textAlign: b.textAlign(c.textAlign)
            }), C ? D.x += i + y : "string" != typeof t.text ? (d = _.lineHeight, D.y += calculateLegendItemHeight(t, d)) : D.y += O
    ...
    drawTitle() {
        ...
        if (s.display) {
            ...
            let t, e = this.left,
                i = this.width;
            this.isHorizontal() ? (i = Math.max(...this.lineWidths), t = this.top + r, e = _alignStartEnd(a.align, e, this.right - i)) : (l = this.columnSizes.reduce((t, e) => Math.max(t, e.height), 0), t = r + _alignStartEnd(a.align, this.top, this.bottom - l - a.labels.padding - this._computeTitleHeight()));
            r = _alignStartEnd(o, e, e + i);
            c.textAlign = h.textAlign(_toLeftRightCenter(o)), c.textBaseline = "middle", c.strokeStyle = s.color, c.fillStyle = s.color, c.font = n.string, renderText(c, s.text, r, t, n)
        }
    }
    _computeTitleHeight() {
    ...
    _getLegendItemAt(t, e) {
        ...
        if (_isBetween(t, this.left, this.right) && _isBetween(e, this.top, this.bottom))
            for (s = this.legendHitBoxes, i = 0; i < s.length; ++i)
                if (a = s[i], _isBetween(t, a.left, a.left + a.width) && _isBetween(e, a.top, a.top + a.height)) return this.legendItems[i];
        return null
    }
    handleEvent(t) {
        var e, i, a, s = this.options;
        isListened(t.type, s) && (e = this._getLegendItemAt(t.x, t.y), "mousemove" === t.type || "mouseout" === t.type ? (i = this._hoveredItem, a = itemsEqual(i, e), i && !a && callback(s.onLeave, [t, i, this], this), (this._hoveredItem = e) && !a && callback(s.onHover, [t, e, this], this)) : e && callback(s.onClick, [t, e, this], this))
    }
}
...
class Title extends Element {
    ...
    update(t, e) {
        var i = this.options;
        this.left = 0, this.top = 0, i.display ? (this.width = this.right = t, this.height = this.bottom = e, t = isArray(i.text) ? i.text.length : 1, this._padding = toPadding(i.padding), e = t * toFont(i.font).lineHeight + this._padding.height, this.isHorizontal() ? this.height = e : this.width = e) : this.width = this.height = this.right = this.bottom = 0
    }
    isHorizontal() {
    ...
    _drawArgs(t) {
        ...
        let o = 0,
            l, h, c;
        return l = this.isHorizontal() ? (h = _alignStartEnd(r, i, s), c = e + t, s - i) : (o = "left" === n.position ? (h = i + t, c = _alignStartEnd(r, a, e), -.5 * PI) : (h = s - t, c = _alignStartEnd(r, e, a), .5 * PI), a - e), {
            titleX: h,
            titleY: c,
            maxWidth: l,
            rotation: o
    ...
    draw() {
        var t, e, i, a, s, n = this.ctx,
            r = this.options;
        r.display && (e = (t = toFont(r.font)).lineHeight / 2 + this._padding.top, {
            titleX: e,
            titleY: i,
            maxWidth: a,
            rotation: s
        } = this._drawArgs(e), renderText(n, r.text, 0, 0, t, {
            color: r.color,
            maxWidth: a,
            rotation: s,
...
const positioners = {
    ...
    nearest(t, e) {
        ...
        for (n = 0, r = t.length; n < r; ++n) {
            const c = t[n].element;
            var l;
            c && c.hasValue() && (l = c.getCenterPoint(), (l = distanceBetweenPoints(e, l)) < s && (s = l, o = c))
        }
        var h;
...
function getTooltipSize(t, e) {
    ...
    var {
        boxWidth: r,
        boxHeight: o
    } = e, l = toFont(e.bodyFont), h = toFont(e.titleFont), c = toFont(e.footerFont), d = n.length, u = s.length, g = a.length, p = toPadding(e.padding);
    let f = p.height,
        m = 0;
    var v = a.reduce((t, e) => t + e.before.length + e.lines.length + e.after.length, 0);
    v += t.beforeBody.length + t.afterBody.length, d && (f += d * h.lineHeight + (d - 1) * e.titleSpacing + e.titleMarginBottom), v && (d = e.displayColors ? Math.max(o, l.lineHeight) : l.lineHeight, f += g * d + (v - g) * l.lineHeight + (v - 1) * e.bodySpacing), u && (f += e.footerMarginTop + u * c.lineHeight + (u - 1) * e.footerSpacing);
    let x = 0;

    function b(t) {
        m = Math.max(m, i.measureText(t).width + x)
    }
    return i.save(), i.font = h.string, each(t.title, b), i.font = l.string, each(t.beforeBody.concat(t.afterBody), b), x = e.displayColors ? r + 2 + e.boxPadding : 0, each(a, t => {
        each(t.before, b), each(t.lines, b), each(t.after, b)
    }), x = 0, i.font = c.string, each(t.footer, b), i.restore(), {
        width: m += p.width,
        height: f
    }
}
...
function doesNotFitWithAlign(t, e, i, a) {
    var {
        x: a,
        width: s
    } = a, i = i.caretSize + i.caretPadding;
...
function getBackgroundPoint(t, e, i, a) {
    ...
    let c = alignX(e, i);
    var d = alignY(e, r, s);
    return "center" === r ? "left" === i ? c += s : "right" === i && (c -= s) : "left" === i ? c -= Math.max(n, l) + t : "right" === i && (c += Math.max(o, h) + t), {
        x: _limitValue(c, 0, a.width - e.width),
        y: _limitValue(d, 0, a.height - e.height)
    }
}
...
class Tooltip extends Element {
    ...
    getBody(t, e) {
        ...
        return each(t, t => {
            ...
            pushOrConcat(e.before, splitNewlines(invokeCallbackWithFallback(i, "beforeLabel", this, t))), pushOrConcat(e.lines, invokeCallbackWithFallback(i, "label", this, t)), pushOrConcat(e.after, splitNewlines(invokeCallbackWithFallback(i, "afterLabel", this, t))), s.push(e)
        }), s
    }
    getAfterBody(t, e) {
    ...
    _createItems(a) {
        ...
        return a.filter && (e = e.filter((t, e, i) => a.filter(t, e, i, s))), a.itemSort && (e = e.sort((t, e) => a.itemSort(t, e, s))), each(e, t => {
    ...
    getCaretPosition(t, e, i) {
        ...
        let d, u, g, p, f, m;
        return "center" === s ? (f = h + c / 2, m = "left" === a ? (d = t, u = d - i, p = f + i, f - i) : (d = t + e, u = d + i, p = f - i, f + i), g = d) : (u = "left" === a ? t + Math.max(n, o) + i : "right" === a ? t + e - Math.max(r, l) - i : this.caretX, g = "top" === s ? (p = h, f = p - i, d = u - i, u + i) : (p = h + c, f = p + i, d = u + i, u - i), m = p), {
            x1: d,
            x2: u,
            x3: g,
            y1: p,
    ...
    drawTitle(t, e, i) {
        ...
        if (s) {
            ...
            for (t.x = getAlignedX(this, i.titleAlign, i), e.textAlign = l.textAlign(i.titleAlign), e.textBaseline = "middle", n = toFont(i.titleFont), r = i.titleSpacing, e.fillStyle = i.titleColor, e.font = n.string, o = 0; o < s; ++o) e.fillText(a[o], l.x(t.x), t.y + n.lineHeight / 2), t.y += n.lineHeight + r, o + 1 === s && (t.y += i.titleMarginBottom - r)
        }
    }
    _drawColorBox(t, e, i, a, s) {
        var n, r = this.labelColors[i],
        ...
        s.usePointStyle ? (d = {
            ...
            rotation: o.rotation,
            borderWidth: 1
        }, o = a.leftForLtr(u, h) + h / 2, n = e + l / 2, t.strokeStyle = s.multiKeyBackground, t.fillStyle = s.multiKeyBackground, drawPoint(t, d, o, n), t.strokeStyle = r.borderColor, t.fillStyle = r.backgroundColor, drawPoint(t, d, o, n)) : (t.lineWidth = isObject(r.borderWidth) ? Math.max(...Object.values(r.borderWidth)) : r.borderWidth || 1, t.strokeStyle = r.borderColor, t.setLineDash(r.borderDash || []), t.lineDashOffset = r.borderDashOffset || 0, d = a.leftForLtr(u, h - c), o = a.leftForLtr(a.xPlus(u, 1), h - c - 2), n = toTRBLCorners(r.borderRadius), Object.values(n).some(t => 0 !== t) ? (t.beginPath(), t.fillStyle = s.multiKeyBackground, addRoundedRectPath(t, {
            x: d,
            y: e,
            w: h,
            h: l,
            radius: n
        }), t.fill(), t.stroke(), t.fillStyle = r.backgroundColor, t.beginPath(), addRoundedRectPath(t, {
            x: o,
            y: e + 1,
            w: h - 2,
            h: l - 2,
    ...
    drawBody(e, i, t) {
        ...
        var c = toFont(t.bodyFont);
        let d = c.lineHeight,
            u = 0;
        const g = getRtlAdapter(t.rtl, this.x, this.width);

        function p(t) {
            i.fillText(t, g.x(e.x + u), e.y + d / 2), e.y += d + s
        }
        var f = g.textAlign(n);
        let m, v, x, b, _, y, k;
        for (i.textAlign = n, i.textBaseline = "middle", i.font = c.string, e.x = getAlignedX(this, f, t), i.fillStyle = t.bodyColor, each(this.beforeBody, p), u = r && "right" !== f ? "center" === n ? l / 2 + h : l + 2 + h : 0, b = 0, y = a.length; b < y; ++b) {
            for (m = a[b], v = this.labelTextColors[b], i.fillStyle = v, each(m.before, p), x = m.lines, r && x.length && (this._drawColorBox(i, e, b, g, t), d = Math.max(c.lineHeight, o)), _ = 0, k = x.length; _ < k; ++_) p(x[_]), d = c.lineHeight;
            each(m.after, p)
        }
        u = 0, d = c.lineHeight, each(this.afterBody, p), e.y -= s
    }
    drawFooter(t, e, i) {
        var a = this.footer,
            s = a.length;
        ...
        if (s) {
            ...
            for (t.x = getAlignedX(this, i.footerAlign, i), t.y += i.footerMarginTop, e.textAlign = o.textAlign(i.footerAlign), e.textBaseline = "middle", n = toFont(i.footerFont), e.fillStyle = i.footerColor, e.font = n.string, r = 0; r < s; ++r) e.fillText(a[r], o.x(t.x), t.y + n.lineHeight / 2), t.y += n.lineHeight + i.footerSpacing
        }
    }
    ...
    draw(t) {
        ...
        if (i) {
            ...
            e.enabled && n && (t.save(), t.globalAlpha = i, this.drawBackground(r, t, a, e), overrideTextDirection(t, e.textDirection), r.y += s.top, this.drawTitle(r, t, e), this.drawBody(r, t, e), this.drawFooter(r, t, e), restoreTextDirection(t, e.textDirection), t.restore())
        }
    }
    getActiveElements() {
        return this._active || []
    ...
    handleEvent(t, e, i = !0) {
        ...
        return s && (this._active = i, (a.enabled || a.external) && (this._eventPosition = {
            x: t.x,
            y: t.y
        }, this.update(!0, e))), s
    }
    ...
    _positionChanged(t, e) {
        var {
            ...
            caretY: a,
            options: s
        } = this, s = positioners[s.position].call(this, t, e);
...
const addIfString = (t, e, i, a) => ("string" == typeof e ? (i = t.push(e) - 1, a.unshift({
    index: i,
    label: e
})) : isNaN(e) && (i = null), i);
...
function generateTicks$1(t, e) {
    ...
    var {
        ...
        min: e,
        max: p
    } = e, f = !isNullOrUndef(n), m = !isNullOrUndef(r), v = !isNullOrUndef(l), c = (p - e) / (c + 1);
    ...
    (k = Math.ceil(p / x) - Math.floor(e / x)) > g && (x = niceNum(k * x / g / u) * u), isNullOrUndef(o) || (b = Math.pow(10, o), x = Math.ceil(x * b) / b), y = "ticks" === a ? (_ = Math.floor(e / x) * x, Math.ceil(p / x) * x) : (_ = e, p), f && m && s && almostWhole((r - n) / s, x / 1e3) ? (k = Math.round(Math.min((r - n) / x, h)), x = (r - n) / k, _ = n, y = r) : v ? (_ = f ? n : _, y = m ? r : y, k = l - 1, x = (y - _) / k) : (k = (y - _) / x, k = almostEquals(k, Math.round(k), x / 1e3) ? Math.round(k) : Math.ceil(k));
    g = Math.max(_decimalPlaces(x), _decimalPlaces(_));
    b = Math.pow(10, isNullOrUndef(o) ? g : o), _ = Math.round(_ * b) / b, y = Math.round(y * b) / b;
    let S = 0;
    for (f && (d && _ !== n ? (i.push({
            value: n
        }), _ < n && S++, almostEquals(Math.round((_ + S * x) * b) / b, n, relativeLabelSize(n, c, t)) && S++) : _ < n && S++); S < k; ++S) i.push({
        value: Math.round((_ + S * x) * b) / b
    });
    return m && d && y !== r ? i.length && almostEquals(i[i.length - 1].value, r, relativeLabelSize(r, c, t)) ? i[i.length - 1].value = r : i.push({
        value: r
    }) : m && y !== r || i.push({
        value: y
    }), i
...
function relativeLabelSize(t, e, {
    ...
    a = toRadians(a), i = (i ? Math.sin(a) : Math.cos(a)) || .001;
...
class LinearScaleBase extends Scale {
    ...
    handleTickRangeOptions() {
        ...
        var n, r, o = t => a = e ? a : t,
            l = t => s = i ? s : t;
        t && (r = sign(a), n = sign(s), r < 0 && n < 0 ? l(0) : 0 < r && 0 < n && o(0)), a === s && (r = 0 === s ? 1 : Math.abs(.05 * s), l(s + r), t || o(a - r)), this.min = a, this.max = s
    }
    getTickLimit() {
        let {
            maxTicksLimit: t,
        ...
        return e ? 1e3 < (i = Math.ceil(this.max / e) - Math.floor(this.min / e) + 1) && (console.warn(`scales.${this.id}.ticks.stepSize: ${e} would result generating up to ${i} ticks. Limiting to 1000.`), i = 1e3) : (i = this.computeTickLimit(), t = t || 11), i = t ? Math.min(t, i) : i
    }
    computeTickLimit() {
...
function generateTicks(t, {
    ...
    let c = Math.floor((s - h) / Math.pow(10, n)),
        d = finiteOrDefault(t.min, Math.round((l + h + c * Math.pow(10, n)) * r) / r);
    for (; d < i;) a.push({
        value: d,
        major: isMajor(d),
        significand: c
    }), 10 <= c ? c = c < 15 ? 15 : 20 : c++, 20 <= c && (n++, c = 2, r = 0 <= n ? 1 : r), d = Math.round((l + h + c * Math.pow(10, n)) * r) / r;
    e = finiteOrDefault(t.max, d);
    return a.push({
        value: e,
        major: isMajor(e),
        significand: c
...
function measureLabelSize(t, e, i) {
    return i = isArray(i) ? i : [i], {
        w: _longestText(t, e.string, i),
...
function fitWithPointLabels(e) {
    ...
    for (let t = 0; t < r; t++) {
        var h = o.setContext(e.getPointLabelContext(t)),
            c = (n[t] = h.padding, e.getPointPosition(t, e.drawingArea + n[t], l)),
            h = toFont(h.font),
            h = measureLabelSize(e.ctx, h, e._pointLabels[t]),
            d = (s[t] = h, _normalizeAngle(e.getIndexAngle(t) + l)),
            u = Math.round(toDegrees(d));
        updateLimits(a, i, d, determineLimits(u, c.x, h.w, 0, 180), determineLimits(u, c.y, h.h, 90, 270))
    }
...
function buildPointLabelItems(e, i, a) {
    ...
    var n = e._pointLabels.length,
        t = e.options,
        r = getTickBackdropHeight(t) / 2,
        o = e.drawingArea,
        l = t.pointLabels.centerPointLabels ? PI / n : 0;
    for (let t = 0; t < n; t++) {
        var h = e.getPointPosition(t, o + r + a[t], l),
            c = Math.round(toDegrees(_normalizeAngle(h.angle + HALF_PI))),
            d = i[t],
            u = yForAngle(h.y, d.h, c),
            c = getTextAlignForAngle(c),
            g = leftForTextAlign(h.x, d.w, c);
        s.push({
            x: h.x,
            y: u,
            textAlign: c,
...
function drawPointLabels(e, i) {
    ...
    for (let t = i - 1; 0 <= t; t--) {
        var n, r, o, l = s.setContext(e.getPointLabelContext(t)),
            h = toFont(l.font),
            {
                x: c,
                y: d,
            ...
            v = l["backdropColor"];
        isNullOrUndef(v) || (n = toTRBLCorners(l.borderRadius), r = toPadding(l.backdropPadding), a.fillStyle = v, v = g - r.left, o = p - r.top, f = f - g + r.width, g = m - p + r.height, Object.values(n).some(t => 0 !== t) ? (a.beginPath(), addRoundedRectPath(a, {
            x: v,
            y: o,
            w: f,
            h: g,
            radius: n
        }), a.fill()) : a.fillRect(v, o, f, g)), renderText(a, e._pointLabels[t], c, d + h.lineHeight / 2, h, {
            color: l.color,
            textAlign: u,
            textBaseline: "middle"
        })
...
class RadialLinearScale extends LinearScaleBase {
    ...
    drawGrid() {
        ...
        const {
            ...
            grid: a,
            border: s
        } = e, n = this._pointLabels.length;
        let r, o, l;
        if (e.pointLabels.display && drawPointLabels(this, n), a.display && this.ticks.forEach((t, e) => {
            ...
            for (t.save(), r = n - 1; 0 <= r; r--) {
                ...
                d && c && (t.lineWidth = d, t.strokeStyle = c, t.setLineDash(h.borderDash), t.lineDashOffset = h.borderDashOffset, o = this.getDistanceFromCenterForValue(e.ticks.reverse ? this.min : this.max), l = this.getPointPosition(r, o), t.beginPath(), t.moveTo(this.xCenter, this.yCenter), t.lineTo(l.x, l.y), t.stroke())
            }
            t.restore()
        }
    }
    ...
    drawLabels() {
        ...
        if (l.display) {
            ...
            let s, n;
            r.save(), r.translate(this.xCenter, this.yCenter), r.rotate(t), r.textAlign = "center", r.textBaseline = "middle", this.ticks.forEach((t, e) => {
                var i, a;
                0 === e && !o.reverse || (i = l.setContext(this.getContext(e)), a = toFont(i.font), s = this.getDistanceFromCenterForValue(this.ticks[e].value), i.showLabelBackdrop && (r.font = a.string, n = r.measureText(t.label).width, r.fillStyle = i.backdropColor, e = toPadding(i.backdropPadding), r.fillRect(-n / 2 - e.left, -s - a.size / 2 - e.top, n + e.width, a.size + e.height)), renderText(r, t.label, 0, -s, a, {
                    color: i.color
                }))
            }), r.restore()
        }
...
function parse(t, e) {
    ...
    let r = e;
    return "function" == typeof a && (r = a(r)), null === (r = isNumberFinite(r) ? r : "string" == typeof a ? i.parse(r, a) : i.parse(r)) ? null : +(r = s ? "week" !== s || !isNumber(n) && !0 !== n ? i.startOf(r, s) : i.startOf(r, "isoWeek", n) : r)
}
...
function addTick(t, e, i) {
    ...
    i ? i.length && ({
        lo: a,
        hi: s
    } = _lookup(i, e), t[i[a] >= e ? i[a] : i[s]] = !0) : t[e] = !0
}
...
function ticksFromTimestamps(t, e, i) {
    ...
    var n = e.length;
    let r, o;
    for (r = 0; r < n; ++r) o = e[r], s[o] = r, a.push({
        value: o,
        major: !1
    });
...
class TimeScale extends Scale {
    ...
    init(t, e = {}) {
        ...
        a.init(e), mergeIf(i.displayFormats, a.formats()), this._parseOpts = {
            parser: i.parser,
            round: i.round,
    ...
    determineDataLimits() {
        ...
        n && r || (o(this._getLabelBounds()), "ticks" === t.bounds && "labels" === t.ticks.source || o(this.getMinMax(!1))), a = isNumberFinite(a) && !isNaN(a) ? a : +e.startOf(Date.now(), i), s = isNumberFinite(s) && !isNaN(s) ? s : +e.endOf(Date.now(), i) + 1, this.min = Math.min(a, s - 1), this.max = Math.max(a + 1, s)
    }
    _getLabelBounds() {
    ...
    initOffsets(t = []) {
        ...
        e = _limitValue(e, 0, a), i = _limitValue(i, 0, a), this._offsets = {
            start: e,
            end: i,
            factor: 1 / (e + 1 + i)
        }
    ...
    _generate() {
        ...
        for (c = h, d = 0; c < i; c = +t.add(c, r, n), d++) addTick(l, c, u);
        return c !== i && "ticks" !== a.bounds && 1 !== d || addTick(l, c, u), Object.keys(l).sort((t, e) => t - e).map(t => +t)
    }
    getLabelForValue(t) {
        const e = this._adapter;
...
function interpolate(t, e, i) {
    let a = 0,
        s = t.length - 1,
        n, r, o, l;
    i ? (e >= t[a].pos && e <= t[s].pos && ({
        lo: a,
        hi: s
    } = _lookupByKey(t, "pos", e)), {
        pos: n,
        time: o
    } = t[a], {
        pos: r,
        time: l
    } = t[s]) : (e >= t[a].time && e <= t[s].time && ({
        lo: a,
        hi: s
    } = _lookupByKey(t, "time", e)), {
        time: n,
        pos: o
    } = t[a], {
        time: r,
...
class TimeSeriesScale extends TimeScale {
    ...
    buildLookupTable(t) {
        ...
        for (n = 0, r = a.length; n < r; ++n) h = a[n + 1], o = a[n - 1], l = a[n], Math.round((h + o) / 2) !== l && s.push({
            time: l,
            pos: n / (r - 1)
        });
        return s
